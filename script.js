...
// ==========================================
// 2. DATA MANAGEMENT (Products & Cart)
// ==========================================
let productsData = []; // Will be filled from Database
let cart = JSON.parse(localStorage.getItem('studySpaceCart')) || [];

// Listen to the Database for Product & Stock updates
onSnapshot(collection(db, "products"), (snapshot) => {
    productsData = snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data()
    }));
    renderAllProducts(); // Re-render whenever stock or prices change
});
...
window.addToCart = function(id) {
    ...
    if (!product || product.stock <= 0) return showToast("Sorry, this item is out of stock!");
    ...
    itemsToShow.forEach(product => {
        ...
        card.innerHTML = `
            ...
            <div class="product-info">
                <h3>${product.name}</h3>
                <p style="font-size: 0.8rem; color: #666; margin-bottom: 5px;">Stock Available: ${product.stock}</p>
                <div class="price-row">
...
window.renderSuccessMessage = function(email) {
    ...
    container.innerHTML = `
        <div class="container text-center" style="padding: 100px 20px;">
            ...
            <p>A confirmation has been sent to <strong>${email}</strong>. Our shipping team has been notified!</p>
            <a href="index.html" class="btn btn-primary" style="margin-top:20px;">Back to Home</a>
        </div>`;
}

// ==========================================
// 7. UI HELPERS (Toasts & Modals)
// ==========================================
